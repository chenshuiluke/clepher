...
const Nightmode = () => {
  ...
  return (
    <div
      className="flex h-12 w-12 transform cursor-pointer items-center justify-center rounded-full transition duration-500 ease-in-out"
      onClick={toggleIcon}
    >
      <div
        className={`transform transition-transform duration-200 ease-in-out ${isMoon ? "rotate-0" : "-rotate-180 scale-0"}`}
      >
        {isMoon && <MoonIcon />}
      </div>
      <div
        className={`transform transition-transform duration-200 ease-in-out ${isMoon ? "rotate-180 scale-0" : "rotate-0"}`}
      >
        {!isMoon && <SunIcon />}
      </div>
    </div>
